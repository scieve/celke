...
if ($id) {
    ...
    if (($resultado_sts_pg) AND ( $resultado_sts_pg->num_rows != 0)) {
        ...
        $result_sts_num_men = "SELECT id, ordem FROM sts_sobs_emps 
        ...
        $result_ins_num_maior = "UPDATE sts_sobs_emps SET
        ...
        $result_ins_num_menor = "UPDATE sts_sobs_emps SET
        ...
        if (mysqli_affected_rows($conn)) {
            $_SESSION['msg'] = "<div class='alert alert-success'>Ordem do sobre empresa editado com sucesso!</div>";
            $url_destino = pg . "/listar/sts_list_sob_emp";
            header("Location: $url_destino");
        } else {
            $_SESSION['msg'] = "<div class='alert alert-danger'>Erro ao editar a ordem do sobre empresa!</div>";
            $url_destino = pg . "/listar/sts_list_sob_emp";
            header("Location: $url_destino");
        }
    } else {
        $_SESSION['msg'] = "<div class='alert alert-danger'>A ordem do sobre empresa não pode ser alterado!</div>";
        $url_destino = pg . "/listar/sts_list_sob_emp";
        header("Location: $url_destino");
    }
} else {
