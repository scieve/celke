...
if ($id) {
    //Pesquisar no banco de dados as informações na tabela sts_pergs_resps
    $result_sts_pg = "SELECT id, ordem
                FROM sts_pergs_resps
                WHERE id='$id' LIMIT 1";
    ...
    if (($resultado_sts_pg) AND ( $resultado_sts_pg->num_rows != 0)) {
        ...
        //Pesquisar o ID do sts_pergs_resps a ser movido para baixo
        $ordem_num_menor = $row_sts_pg['ordem'] - 1;
        $result_sts_num_men = "SELECT id, ordem FROM sts_pergs_resps 
        ...
        $result_ins_num_maior = "UPDATE sts_pergs_resps SET
        ...
        $result_ins_num_menor = "UPDATE sts_pergs_resps SET
        ...
        if (mysqli_affected_rows($conn)) {
            $_SESSION['msg'] = "<div class='alert alert-success'>Ordem da pergunta e resposta editado com sucesso!</div>";
            $url_destino = pg . "/listar/sts_list_perg_resp";
            header("Location: $url_destino");
        } else {
            $_SESSION['msg'] = "<div class='alert alert-danger'>Erro ao editar a ordem da pergunta e resposta!</div>";
            $url_destino = pg . "/listar/sts_list_perg_resp";
            header("Location: $url_destino");
        }
    } else {
        $_SESSION['msg'] = "<div class='alert alert-danger'>A ordem da pergunta e resposta não pode ser alterado!</div>";
        $url_destino = pg . "/listar/sts_list_perg_resp";
        header("Location: $url_destino");
    }
} else {
