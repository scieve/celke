...
$SendEditSobEmp = filter_input(INPUT_POST, 'SendEditSobEmp', FILTER_SANITIZE_STRING);
if ($SendEditSobEmp) {
    ...
    if (!$dados_validos) {
        $erro = true;
        $_SESSION['msg'] = "<div class='alert alert-danger'>Necessário preencher todos os campos para editar o item sobre empresa!</div>";
    } 
    ...
    if ($erro) {
        ...
        $result_pg_up = "UPDATE sts_sobs_emps SET 
        ...
        if (mysqli_affected_rows($conn)) {
            ...
            if (!empty($foto['name'])) {
                ...
                $destino = "../assets/imagens/sob_emp/" . $dados['id'] . "/";
            ...
            $_SESSION['dados'] = $dados;
            $_SESSION['msg'] = "<div class='alert alert-danger'>Erro: Sobre empresa não foi editado!</div>";
            $url_destino = pg . '/editar/sts_edit_sob_emp?id='.$dados['id'];
            header("Location: $url_destino");
        }
    }
